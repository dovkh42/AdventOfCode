...
rl.on('close', () => {
    ...
    const getNextPassword = (inputLine) => {
        ...
        return convertAsciiArrayToChar(asciiArray);
    }

    let oldPassword = getNextPassword(inputLine);
    oldPassword = convertCharArrayToAscii(oldPassword);
    incrementPassword(oldPassword)
    oldPassword = convertAsciiArrayToChar(oldPassword)

    //console.log(oldPassword.join(''))

    result = getNextPassword(oldPassword).join('');
